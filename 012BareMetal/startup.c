 ...
 * object files. By default, without any annotation, a symbol in an object file is strong. During linking,
 * a strong symbol can override a weak symbol of the same name. In contrast, in the presence of two strong 
 * symbols by the same name, the linker resolves the symbol in favor of the first one found. This behavior
 * allows an executable to override standard library functions, such as malloc(3). When linking a binary
 * executable, a weakly declared symbol does not need a definition. In comparison, (by default) a declared
 * strong symbol without a definition triggers an undefined symbol link error.
 */
...
/* vector table 
 * place the vecor table inside a special section (.isr_vector).
 * a zero (0) means that memory space is reserved. The entry must be initialized to make sure all
 * memory areas are initialized and the data is alined correctly
 *
 * this startup file is make for a stm32, it can variate with a different chip (type)
 */
...
void Reset_handler(void)
{
	...
	uint32_t size = (uint32_t)&__END_OF_DATA__ - (uint32_t)&__START_OF_DATA__;
...
/* dummy function
 * must be overwritten by the programmer
 */
